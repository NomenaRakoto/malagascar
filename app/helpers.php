...
if(!function_exists('profil_note')) {
	function profil_note($avis){
	    $note = 0;
	    $nb = 0;
	    $moyenne = 0;
...
if(!function_exists('month')) {
	function month($m){
	    $months = [
	    	'mg' => ['Janoary', 'Febroary', 'Martsa', 'Aprily', 'May', 'Jona', 'Jolay', 'Aogositra', 'Septambra', 'Oktobra', 'Novambra', 'Desambra'],
...
if(!function_exists('is_pj_verified')) {
	function is_pj_verified($user, $pj){
	   foreach ($user->pieces as $key => $piece) {
	   		if(str_contains($piece->pj_name, $pj) && $piece->is_verified) {
	   			return true;
	   		}
	   }
	   return false;
...
if(!function_exists('nb_note')) {
	function nb_note($user, $note){
	   $nb = 0;
	   foreach ($user->profil_avis as $key => $avis) {
	   		if($avis->note == $note) {
	   			$nb++;
	   		}
	   }
...
if(!function_exists('note')) {
	function note($note){
	   $notes = ['Tres decevant', 'decevant', 'Correct', 'Bien', 'Excellent'];
	   return $notes[$note-1];
	}
}
